...
word_disponibles = random.sample(categorias[categoria], len(categorias[categoria]))
puntaje_total = 0

while word_disponibles:
    word = word_disponibles.pop().lower()
    guessed = []
    attempts = 6
    puntaje = 0
    print()
    ...
    if word_disponibles and input("¿Quieres jugar otra vez? (s/n): ").lower() == "s":
        print()
        print("---------------- Nueva ronda ------------------")
    else:
        print(f"Gracias por jugar! Tu puntaje total fue: {puntaje_total}")
        break